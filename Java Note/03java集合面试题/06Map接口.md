...
详细内容可以看 [[03集合容器概述#8 哪些集合类是线程安全的？]] 里的 ConcurrentHashMap 深度解析。
...
能。这就是 [[06Map接口#2 什么是 Hash 算法？|哈希冲突]]。比如 `"Aa"` 和 `"BB"` hashCode 都是 2112。
...
- **modCount** +1：记录结构修改次数，给 fail-fast 用的（[[03集合容器概述#9 Java集合的快速失败机制 fail-fast|回顾 fail-fast]]）
- **size** +1：元素个数
- 如果 `size > threshold`（容量 × 负载因子）→ **扩容**（详细过程见 [[06Map接口#扩容机制|4. 扩容机制]]，JDK 1.7/1.8 区别见 [[06Map接口#区别三扩容时的 rehash 方式|5. 扩容 rehash 区别]]）
...
- **覆盖 vs 新增**：覆盖返回旧值、不改 modCount、不触发扩容检查（扩容细节见 [[06Map接口#扩容机制|4. 扩容机制]]）；新增才++size、检查 threshold
...
具体细节可以回顾 [[06Map接口#5 HashMap 在 JDK 1.7 和 1.8 中有哪些不同？还有哪些集合有类似更新？|第 5 题 JDK 7 vs 8 区别]] 和 [[06Map接口#7 什么是红黑树？|第 7 题红黑树]]。
...
这部分在 [[06Map接口#8 HashMap 的 put 方法具体流程？|第 8 题 put 流程]] 的步骤 ③ 和 ⑤ 有详细代码。
...
具体原理见 [[06Map接口#6 hash 值、hashCode、key 有什么区别？|第 6 题 hash 值 vs hashCode]]。
...
细节见 [[06Map接口#4 说一下 HashMap 的实现原理？|第 4 题 为什么要用 & 而不是 %]]。
...
这也是 [[06Map接口#4 说一下 HashMap 的实现原理？|第 4 题]] 里提到过的。
...
扩容相当于给元素**重新分座位**——原本挤在一起的可能就分开了。这就是 [[06Map接口#9 HashMap 的扩容操作是怎么实现的？|第 9 题扩容]] 里讲的 `e.hash & oldCap` 的效果。
...
这个机制在 [[06Map接口#9 HashMap 的扩容操作是怎么实现的？|第 9 题扩容]] 里详细讲过。
...
详细见 [[03集合容器概述#8 哪些集合类是线程安全的？]] 的 ConcurrentHashMap 部分。
...
这些在 [[06Map接口#15 HashMap 的长度为什么是 2 的幂次方？|第 15 题]] 详细讲过。
...
详细深入内容在 [[03集合容器概述#8 哪些集合类是线程安全的？]]。
...
put、get、扩容 transfer、size 计数等完整源码分析在 [[03集合容器概述#8 哪些集合类是线程安全的？|03 集合容器概述#8]] 里。
